    ...
    public class RegistrationController : Controller
    {
        ...
        public async Task<IActionResult> Register(Register reg)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError("Password", "Пароль должен состоять из 6 символов и больше.");
                    return View("~/Views/Music/Register.cshtml", reg);
                }
                else if (!(reg.Password.Any(char.IsUpper) && reg.Password.Any(char.IsDigit)))
                {
                    ModelState.AddModelError("Password", "Пароль должен содержать одну заглавную букву и одну цифру.");
                    return View("~/Views/Music/Register.cshtml", reg);
                }
